...
for sn = sn_list
    ...
    for zz=1:length(SPM.Sess)  % for each run
      ...
      for yy=1:length(SPM.Sess(zz).U)    % for each condition
        design{zz}(round(SPM.Sess(zz).U(yy).ons)+1,yy) = 1;  % set all of the onsets
      end
    end
    ...
    for i = 1:length(D.eventname)
        tmp_name = D.eventname{i};
        tmp_name = split(tmp_name,'/');
        tmp_name = tmp_name{1};
        if contains(tmp_name,'UL') % unimanual left:
            tmp_idx = str2double(tmp_name(3));
            D.eventname{i} = sprintf('lhand:%d',angle(tmp_idx));
        end
        if contains(tmp_name,'UR') % unimanual Right:
            tmp_idx = str2double(tmp_name(3));
            D.eventname{i} = sprintf('rhand:%d',angle(tmp_idx));
        end
        if contains(tmp_name,'B') % bimanual:
            tmp_idx1 = str2double(tmp_name(2));
            tmp_idx2 = str2double(tmp_name(4));
            D.eventname{i} = sprintf('bi:%d_%d',angle(tmp_idx1),angle(tmp_idx2));
        end
    end
