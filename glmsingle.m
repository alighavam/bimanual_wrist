...
for sn = sn_list
    ...
    tmp = data{1};
    vol_dims = size(tmp);
    num_voxels = vol_dims(1) * vol_dims(2) * vol_dims(3); % 90 * 90 * 32 = 259200
    ...
    for ii=1:length(designSINGLE)
        data_run = data{ii};
        vol_dims = size(data_run);
        num_voxels = vol_dims(1) * vol_dims(2) * vol_dims(3); % 90 * 90 * 32 = 259200
        num_timepoints = vol_dims(4); % 740
        data_run = reshape(data_run, num_voxels, num_timepoints);
        ...
        % --- B. Generate the Polynomial Regressors ---
        num_timepoints = size(data_run, 2);
        run_duration_min = (num_timepoints * tr) / 60;
        poly_degree = round(run_duration_min / 2);
        ...
        maskindices = any(data_run ~= 0, 2);
        data_run = double(data_run(maskindices, :)); % voxels x time data
        
        betas_nuisance = nuisance_regressors \ data_run'; 
        y_noise = nuisance_regressors * betas_nuisance;
        y_adj(:,(ii-1)*740+1:ii*740) = (data_run' - y_noise)';
    end
    ...
    % Extract all necessary data for ALL voxels at once using these indices:
    fprintf('Extracting model data for all voxels...\n');
    num_voxels = sum(maskindices);
    num_betas = size(typeD.modelmd, 4);

    % Reshape the 4D beta matrix into 2D (all_voxels_in_volume x betas)
    % and then select the voxels we want:
    all_betas_flat = reshape(typeD.modelmd, [], num_betas);
    all_betas = all_betas_flat(maskindices, :); % [num_voxels, num_betas]

    % Extract the other per-voxel information.
    all_hrf_indices = typeD.HRFindex(maskindices);
    all_meansignal = typeD.meanvol(maskindices);
